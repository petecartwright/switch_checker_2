    ...
    for store in stock:
        ...
        response = client.put_item(
            ...
            Item={
                ...
                "city": {"S": store["city"]},
                "state": {"S": store["state"]},
                "postalCode": {"S": str(store["postalCode"])},
...
def fetch_data(query_params):
    ...
    raw_skus = query_params.get("skus") or os.getenv("DEFAULT_SKUS")
    if raw_skus:
    ...
    for sku in skus:
        ...
        if stock_near_zip:
            add_stock_to_table(stock=stock_near_zip, sku=sku, zip_code=zip_code)
